...
export default function ProjectTimeline({ tasks }: Props) {
  ...
  const datedTasks = sorted.filter((t) =>
    t.startDate || t.dueDate || t.createdAt
  );
  if (datedTasks.length === 0) {
    return (
      <div class="pt-empty">
        ...
        <p>
          Set start and due dates on tasks to visualize them on the timeline.
        </p>
      </div>
    );
  }
  ...
  while (cursor < timelineEnd) {
    ...
    const weekEnd = cursor > timelineEnd
      ? new Date(timelineEnd)
      : new Date(cursor);
    weeks.push({
      start: weekStart,
      end: weekEnd,
      label: weekStart.toLocaleDateString("default", {
        month: "short",
        day: "numeric",
      }),
    });
  ...
  const todayX =
    ((now.getTime() - timelineStart.getTime()) / (totalDays * 86400000)) * 100;
    ...
    <div class="pt-container">
      <div class="pt-header">
        <div class="pt-header-left">
          <h3 class="pt-title">
            <Icon name="timeline" size={18} /> Project Timeline
          </h3>
          <span class="pt-count">
        ...
        <div class="pt-legend">
          <span class="pt-legend-item">
            <span class="pt-legend-dot" style="background:var(--success)" />
            {" "}
            Done
          </span>
          <span class="pt-legend-item">
            <span class="pt-legend-dot" style="background:var(--info)" />{" "}
            In Progress
          </span>
          <span class="pt-legend-item">
            <span class="pt-legend-dot" style="background:var(--muted)" /> To Do
          </span>
        </div>
      </div>
        ...
        <div class="pt-rows">
          {datedTasks.map((task) => {
            const taskStart = task.startDate || task.createdAt ||
              task.dueDate || allDates[0];
            const taskEnd = task.dueDate || task.startDate || task.createdAt ||
              allDates[allDates.length - 1];
            const startPos = Math.max(
              0,
              ((new Date(taskStart).getTime() - timelineStart.getTime()) /
                (totalDays * 86400000)) * 100,
            );
            const endPos = Math.min(
              100,
              ((new Date(taskEnd).getTime() - timelineStart.getTime()) /
                (totalDays * 86400000)) * 100,
            );
              ...
              <div key={task.id} class="pt-row">
                <div class="pt-row-info">
                  ...
                  <div class="pt-row-dates">
                    {taskStart && (
                      <span>{new Date(taskStart).toLocaleDateString()}</span>
                    )}
                    <span>→</span>
                    {taskEnd && (
                      <span>{new Date(taskEnd).toLocaleDateString()}</span>
                    )}
                  </div>
                </div>
                <div class="pt-bar-track">
                  <div
                    class="pt-bar"
                    style={`left:${startPos}%;width:${width}%;background:${color}${
                      isOverdue ? ";opacity:0.8" : ""
                    }`}
                  >
